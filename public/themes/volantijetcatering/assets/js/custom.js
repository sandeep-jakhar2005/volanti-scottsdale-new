    ...
    jQuery('body').on('click', '.checkAgeBtn', function () {
        ...
        let selectedYear = dobDate.getFullYear();
        ...
        $("#dobMessage").text("Please select a valid date of birth.");
        return;
        }
    ...
    jQuery('body').on('click', '.UpdateQuantityButton', function () {
        ...
        if (updateQuantityButton.hasClass('loading')) {
            return false;
        }

        updateQuantityButton.addClass('loading');
        ...
        let token = $('meta[name="csrf-token"]').attr('content');
        let $originalButtonHtml = updateQuantityButton.html();

        if (quantity <= 0) {
            updateQuantityButton.removeClass('loading');
            return false;
        }
        ...
        updateQuantityButton.html('<span class="save-btn-ring" style="display:inline-block;"></span>');
    ...
    $('body').on('input', '#phone', function () {
        var digits = $(this).val().replace(/\D/g, ''); // Only numbers
        ...
        // Basic (999) 999-9999 formatting for first 10 digits
        if (digits.length <= 3) {
            formatted = digits;
        } 
