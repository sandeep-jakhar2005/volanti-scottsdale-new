...
function update_mini_cart(response, type = null) {
    ...
    for (const item of cartItem) {
        ...
        if ($existingItem.length > 0) {
            ...
            '                    <span class="bin-icon">' +
            '                        <img src="/themes/volantijetcatering/assets/images/red_trash.png" alt="Bin Icon" width="20" height="20" class="bin-icon-image custom-remove-item ml-2" data-item-id="' + itemId + '">' +
            '                    </span>' +
            '                    </span>' +
            '        </div>' +
            '    </div>' +
