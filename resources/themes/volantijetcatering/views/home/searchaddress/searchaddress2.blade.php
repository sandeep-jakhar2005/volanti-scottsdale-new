...
@push('scripts')
    <script>
     ...
     var delivery_address = jQuery("#auto_search").val();
                ...
                if(!airport_id){
                    $('#airport_select_searchbar').addClass('homepage_error_focus');
                    setTimeout(() => {
                        $('#airport_select_searchbar').removeClass('homepage_error_focus');
                    }, 3000);
                    return;
                }

                if(!selected_fbo_id){
                    $('.airport_fbo_name').addClass('homepage_error_focus');
                    setTimeout(() => {
                        $('.airport_fbo_name').removeClass('homepage_error_focus');
                    }, 3000);
                    return;
                }

                if(!date){
                    $('#time_search_label').addClass('homepage_error_focus');
                    setTimeout(() => {
                        $('#time_search_label').removeClass('homepage_error_focus');
                    }, 3000);
                    return;
                }

                if(!time){
                    $('.delivery_time_slot').addClass('homepage_error_focus');
                    setTimeout(() => {
                        $('.delivery_time_slot').removeClass('homepage_error_focus');
                    }, 3000);
                    return;
                }

                if(address_id && selected_fbo_id && date && time){
                let originalContent = $(this).html();
                $(this).html('<span class="btn-ring"></span>');
                $(this).find(".btn-ring").show();
